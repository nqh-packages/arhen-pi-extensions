...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool<typeof SubagentParams, RunDetails>({
		...
		promptGuidelines: [
			...
			"Define each agent inline: a name, required `task`, explicit `model`, and tool allowance (`write: true`/`write: false`/`tools: [...]`). Optional `prompt` adds system instructions, not the task. A matched agent file takes over (see description); matching is by description, not name — name the agent whatever fits the goal.",
